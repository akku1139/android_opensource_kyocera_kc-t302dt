...
static void msm_anlg_cdc_boost_on(struct snd_soc_codec *codec)
{
	...
	if (get_codec_version(sdm660_cdc) < CAJON_2_0)
		...
		snd_soc_write(codec, MSM89XX_PMIC_ANALOG_CURRENT_LIMIT, 0xA3);
#else
...
#ifdef CONFIG_KYOCERA_MSND
static int msm_anlg_cdc_hp_gain_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	u8 hp_gain;
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	...
	if (get_codec_version(sdm660_cdc) >= DIANGU) {
		hp_gain = snd_soc_read(codec, MSM89XX_PMIC_ANALOG_RX_COM_BIAS_DAC);
		hp_gain = (hp_gain >> 1) & 0x3;

		if (hp_gain == 0x00) {
			ucontrol->value.integer.value[0] = 3;
		} else if (hp_gain == 0x01) {
			ucontrol->value.integer.value[0] = 2;
		} else if (hp_gain == 0x02) {
			ucontrol->value.integer.value[0] = 1;
		} else if (hp_gain == 0x03) {
			ucontrol->value.integer.value[0] = 0;
		} else {
			dev_err(codec->dev,
				"%s: ERROR: Unsupported HP Gain = 0x%x\n",
				__func__, hp_gain);
			return -EINVAL;
		}
	} else {

		hp_gain = snd_soc_read(codec, MSM89XX_PMIC_ANALOG_RX_EAR_CTL);
...
static int msm_anlg_cdc_hp_gain_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	u8 hp_gain;
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
...
#ifdef CONFIG_KYOCERA_MSND
static const char * const msm_anlg_cdc_hp_gain_text[] = {
		"POS_M4P5_DB", "POS_M3_DB", "POS_M1P5_DB", "POS_0_DB"};
static const struct soc_enum msm_anlg_cdc_hp_gain_enum[] = {
		SOC_ENUM_SINGLE_EXT(4, msm_anlg_cdc_hp_gain_text),
};
#endif
...
#ifdef CONFIG_KYOCERA_MSND
	SOC_ENUM_EXT("HP Gain", msm_anlg_cdc_hp_gain_enum[0],
		msm_anlg_cdc_hp_gain_get, msm_anlg_cdc_hp_gain_put),
#endif
...
static const struct sdm660_cdc_reg_mask_val cajon2p0_wcd_reg_defaults[] = {
	...
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_TX_1_2_OPAMP_BIAS, 0x4C),
#ifdef CONFIG_KYOCERA_MSND
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_CURRENT_LIMIT, 0xA3),
#else
	...
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_SPKR_DRV_DBG, 0x01),
#ifdef CONFIG_KYOCERA_MSND
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_SPKR_OCP_CTL, 0xA1),
#else
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_SPKR_OCP_CTL, 0xE1),
#endif
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_SPKR_DAC_CTL, 0x03),
	...
	MSM89XX_REG_VAL(MSM89XX_PMIC_DIGITAL_CDC_RST_CTL, 0x80),
#ifdef CONFIG_KYOCERA_MSND
	MSM89XX_REG_VAL(MSM89XX_PMIC_ANALOG_MICB_1_INT_RBIAS, 0x4D),
#endif
};
