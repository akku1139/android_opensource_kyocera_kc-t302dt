...
#ifdef CONFIG_KYOCERA_MSND
static struct wake_lock mbhc_wakelock;
#endif
...
EXPORT_SYMBOL(wcd_mbhc_get_jack_status);
...
#ifdef CONFIG_KYOCERA_MSND
	wake_lock_timeout(&mbhc_wakelock, MBHC_WAKELOCK_TIME);
#endif
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->mbhc_detection_logic == WCD_DETECTION_LEGACY &&
		(mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) && (mbhc->hs_detect_work_stop)) {
#else
	if (mbhc->mbhc_detection_logic == WCD_DETECTION_LEGACY &&
		mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
#endif
...
#ifdef CONFIG_KYOCERA_MSND
	wake_lock_init(&mbhc_wakelock, WAKE_LOCK_SUSPEND, "mbhc");
#endif
...
#ifdef CONFIG_KYOCERA_MSND
	wake_lock_destroy(&mbhc_wakelock);
#endif
}
