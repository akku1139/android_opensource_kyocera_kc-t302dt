...
#ifdef CONFIG_KYOCERA_MSND
static struct device_node *kc_of_node;
#endif
...
#ifdef CONFIG_KYOCERA_MSND
	u16 btn_low_prop[5], btn_high_prop[5];
	int ret = 0;
#endif
...
#undef S
	...
	btn_high[4] = 500;
#ifdef CONFIG_KYOCERA_MSND
	ret =  of_property_read_u16_array(kc_of_node, "kc,btn-low-th", btn_low_prop, 5);
	ret |= of_property_read_u16_array(kc_of_node, "kc,btn-high-th", btn_high_prop, 5);
	if (ret == 0) {
		int i;
		pr_debug("%s: Btn threshold use kc dts value.\n", __func__);
		for (i = 0; i < 5; i++) {
			btn_low[i] = btn_low_prop[i];
			btn_high[i] = btn_high_prop[i];
			pr_debug("%s: btn_low[%d]=%d, btn_high[%d]=%d\n", __func__, i, btn_low[i], i, btn_high[i]);
		}
	}
#endif
