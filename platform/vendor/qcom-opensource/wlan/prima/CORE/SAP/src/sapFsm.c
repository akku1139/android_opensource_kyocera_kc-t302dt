...
)
{
...
#endif
    v_U8_t firstChannel = 0;
    v_U8_t loopChannelCount;
    tHalHandle hHal;
    tANI_U8   channel;
    ...
    if (sapContext->channel == AUTO_CHANNEL_SELECT)
    {
        ...
        firstChannel =  *channelList;
        for(loopChannelCount = 0; loopChannelCount < numOfChannels - 1; loopChannelCount++) {
            *(channelList + loopChannelCount)= *(channelList + loopChannelCount +1);
        }
        *(channelList + numOfChannels - 1) = firstChannel;
